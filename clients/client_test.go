...
func (m *ResponseProcessMethod) ResponseProcess(body io.ReadCloser, h http.Header, s StatusCode) (*Response, error) {
	...
	return &Response{Text: buf.String(), StatusCode: s}, nil
}
...
func (m *FailedMethod) ResponseProcess(body io.ReadCloser, h http.Header, s StatusCode) (*Response, error) {
	buf := new(bytes.Buffer)
	n, err := buf.ReadFrom(body)
	if n <= 0 {
		return &Response{StatusCode: s}, EmptyPageError
...
func (m *ValidFailedMethod) ResponseProcess(body io.ReadCloser, h http.Header, s StatusCode) (*Response, error) {
	buf := new(bytes.Buffer)
	n, err := buf.ReadFrom(body)
	if n <= 0 {
		return &Response{StatusCode: s}, EmptyPageError
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("NetClient.Request() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if got != nil && (got.StatusCode != tt.want.code || got.CountRetry != tt.want.countRetry) {
				t.Errorf("NetClient.Request() = %v, want %v", got, tt.want)
			}
		})
	}
}
